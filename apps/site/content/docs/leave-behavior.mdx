...
import { Heart, Send } from "lucide-motion";
...
If the icon sits inside a parent with a hover color transition, the parent's color will release the moment the cursor leaves while the stroke is still drawing. To keep the host color pinned for the full draw, use the [`data-motion-state` attribute](/docs/motion-state) the icon broadcasts on its `<svg>`.
